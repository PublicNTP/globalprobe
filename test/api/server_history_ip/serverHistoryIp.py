...
def _attemptServerHistory(logger, newServers, dbConnection, dbCursor):
    ...
    serverHistoryEndpoint = "https://25zwa0yf5h.execute-api.us-east-2.amazonaws.com/dev/v1/server/history/last_n_secs/86400/ip_address"

    for currServerName in newServers:
        currServer = newServers[currServerName]

        for currIP in currServer['server_addresses']:
            cognitoTokens = _getCognitoUserTokens()
            authHeaders = {
                "Authorization": cognitoTokens['identity']
            }

            historyAttempt = requests.get( "{0}/{1}".format(serverHistoryEndpoint, currIP), headers=authHeaders)
            ...
            if historyAttempt.status_code == 200:
                ...
                logger.error("Could not get history for {0} address {1}".format(currServerName, currIP))
...
def _getNewServerData():
    serversToAdd = {
        'unittest-history.globalprobe.dev.publicntp.org': {
