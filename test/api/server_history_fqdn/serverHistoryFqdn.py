...
def _attemptServerHistory(logger, newServers, dbConnection, dbCursor):
    ...
    serverHistoryEndpoint = "https://25zwa0yf5h.execute-api.us-east-2.amazonaws.com/dev/v1/server/history/last_n_secs/86400/fqdn"
    ...
    for currServerName in newServers:
        ...
        historyAttempt = requests.get( "{0}/{1}".format(serverHistoryEndpoint, currServerName), headers=authHeaders)

        if historyAttempt.status_code == 200:
            logger.info("API claims it got history for {0}".format(currServerName) )
