...
def _attemptAlertList(logger, dbConnection, dbCursor):

    listAlertsEndpoint = "https://25zwa0yf5h.execute-api.us-east-2.amazonaws.com/dev/v1/alert/list"
    ...
    listAttempt = requests.get( listAlertsEndpoint, headers=authHeaders)

    if listAttempt.status_code == 200:
        logger.info("API claims it listed alerts" )

        returnedBody = listAttempt.json()
        alertList = returnedBody['alerts']


        logger.debug(json.dumps(alertList))
    ...
    try:
        with psycopg2.connect("dbname='{0}' user='{1}' host='{2}' password='{3}'".format(
            ...
            with dbConnection.cursor() as dbCursor:
                ...
                _attemptAlertList(logger, dbConnection, dbCursor)
                #_clearTestData(logger, dbConnection, dbCursor)
