...
def _attemptAlertDelete(logger, dbConnection, dbCursor):

    deleteAlertsEndpoint = "https://25zwa0yf5h.execute-api.us-east-2.amazonaws.com/dev/v1/alert/"
    ...
    alertsToDelete = [ 12345, 67890, 1034234 ]

    for i in range(len(alertsToDelete)):

        deleteAttempt = requests.delete( deleteAlertsEndpoint + str(alertsToDelete[i]), 
            headers=authHeaders )

        if deleteAttempt.status_code == 200:
            logger.info("API claims it deleted alert" )

        else:
            logger.error("Could not delete alert")
    ...
    try:
        with psycopg2.connect("dbname='{0}' user='{1}' host='{2}' password='{3}'".format(
            ...
            with dbConnection.cursor() as dbCursor:
                ...
                _attemptAlertDelete(logger, dbConnection, dbCursor)
                #_clearTestData(logger, dbConnection, dbCursor)
